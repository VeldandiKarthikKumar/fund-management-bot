...
class TradeSuggestion(Base):
    ...
    signals_fired   = Column(JSON, nullable=False)        # [{"name": "ema_crossover", "strength": 0.8, ...}]
    ...
    market_context  = Column(JSON)   # {"nifty_trend": "up", "vix": 14.2, "sector": "bullish"}

    # Slack threading
    slack_ts        = Column(String(50), index=True)      # Message timestamp (used to update/thread)
...
class DailyJournal(Base):
    ...
    __table_args__ = (
        UniqueConstraint("date", name="uq_journal_date"),
    )
